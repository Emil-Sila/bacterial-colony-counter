  ...
  /* [Designer("Cyotek.Windows.Forms.Design.ImageBoxDesigner", Cyotek.Windows.Forms.ImageBox.Design.dll, PublicKeyToken=58daa28b0b2de221")] */ public class ImageBox : VirtualScrollableControl
  {
    ...
    public virtual void ZoomToFit()
    {
      if (!this.ViewSize.IsEmpty)
      {
        ...
        _zoomToFit = (int)Math.Round(Math.Floor(zoom));
        this.Zoom = _zoomToFit;
      }
    }
    ...
    protected virtual void ProcessMouseZoom(bool isZoomIn, Point cursorPosition)
    {
      ...
      if (isZoomIn)
      {
          this.SetZoom(this.ZoomLevels.NextZoom(this.Zoom), ImageBoxZoomActions.ZoomIn, ImageBoxActionSources.User);
      }
      // Zoom out if the image doesn't fit the entire imageBox
      else if (this.ZoomLevels.PreviousZoom(this.Zoom) > _zoomToFit)
      {
          this.SetZoom(this.ZoomLevels.PreviousZoom(this.Zoom), ImageBoxZoomActions.ZoomOut, ImageBoxActionSources.User);
      }
      // Don't allow zooming out if the image fits the entire imageBox
      else
      {
          this.Zoom = _zoomToFit;
      }
    ...
    /// </param>
    public virtual void ProcessPanning(MouseEventArgs e)
    {
